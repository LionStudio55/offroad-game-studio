...
public class RCC_CarControllerV3 : RCC_Core {
    ...
    public float fuelConsumptionRate = 0.1f;         // Fuel Consumption Rate.
    #endregion
    ...
    private void Inputs_mobile()
    {
        ...
        if (canControl)
        {
            ...
            if (!externalController)
            {
                ...
                inputs = RCC_InputManager.GetInputs(); //by uzair

                if (!automaticGear || semiAutomaticGear)
                {
                    if (!changingGear && !cutGas)
                        throttleInput = HUDListner.Throtle/*inputs.throttleInput*/;
                    else
                        throttleInput = 0f;
                }
                else
                {
                    if (!changingGear && !cutGas)
                        throttleInput = (direction == 1 ? Mathf.Clamp01(HUDListner.Throtle/*inputs.throttleInput*/) : Mathf.Clamp01(HUDListner.Brake/*inputs.brakeInput*/));
                    else
                        throttleInput = 0f;
                }
                ...
                if (!automaticGear || semiAutomaticGear)
                {
                    brakeInput = Mathf.Clamp01(HUDListner.Brake/*inputs.brakeInput*/);
                }
                else
                {
                    if (!cutGas)
                        brakeInput = (direction == 1 ? Mathf.Clamp01(HUDListner.Brake/*inputs.brakeInput*/) : Mathf.Clamp01(HUDListner.Throtle/*inputs.throttleInput*/));
                    else
                        brakeInput = 0f;
                }
                ...
                if (useSteeringSensitivity)
                {
                    ...
                    bool oppositeDirection = Mathf.Sign(HUDListner.steering/*inputs.steerInput*/) != Mathf.Sign(steerInput) ? true : false;
                    steerInput = Mathf.MoveTowards(steerInput, /*inputs.steerInput*/ HUDListner.steering + counterSteerInput, (Time.deltaTime * steeringSensitivityFactor * Mathf.Lerp(10f, 5f, steerAngle / orgSteerAngle)) * (oppositeDirection ? 1f : 1f));

                }
                else
                {
                    steerInput = HUDListner.steering/*inputs.steerInput*/ + counterSteerInput;
                }

                SteeringAssistance();

                boostInput = HUDListner.Boostinput/*inputs.boostInput*/;
                handbrakeInput = HUDListner.Handbrake/*inputs.handbrakeInput*/;
    ...
    private void Fuel() 
    {
        if (speed > 10)
        {
            fuelTank -= ((engineRPM / 10000) * fuelConsumptionRate + 0.25f) * Time.fixedDeltaTime; // edit by RUT Addin extra value in fuel consumption
           //  print("Fueltank :" + ((engineRPM / 10000) * fuelConsumptionRate + 0.25f) * Time.fixedDeltaTime);
